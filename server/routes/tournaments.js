...
export async function renderStandingsImage(t) {
  ...
  const rows = standings.map((s, i) => {
    ...
    const deckArt = [0, 1, 2].map((n) => {
      ...
      const extras = c.small.slice(0, 4).map((p, k) => { const sx = x + 34 + (k % 2) * 23; const sy = y + 28 + Math.floor(k / 2) * 27; const label = xml((p.abbrev || p.name || '?').slice(0, 2).toUpperCase()); return `<circle cx="${sx}" cy="${sy}" r="11" class="piece-ring"/>${asset(p) ? `<image href="${asset(p)}" x="${sx - 11}" y="${sy - 11}" width="22" height="22" preserveAspectRatio="xMidYMid meet" clip-path="url(#pieceClip${i}-${n}-${k})"/>` : `<text x="${sx}" y="${sy + 3}" text-anchor="middle" class="piece-fallback">${label}</text>`}`; }).join('');
      const comboLabel = [c.ratchet?.abbrev || c.ratchet?.name, c.bit?.abbrev || c.bit?.name].filter(Boolean).join(' ');
      return primary + extras + (comboLabel ? `<text x="${x + 34}" y="${y + 80}" text-anchor="middle" class="combo-label">${xml(comboLabel)}</text>` : '');
    }).join('');
  ...
  const defs = standings.map((_, i) => `<clipPath id="avatarClip${i}"><circle cx="123" cy="${208 + i * 98}" r="31"/></clipPath>${[0, 1, 2].map((n) => `<clipPath id="bladeClip${i}-${n}"><circle cx="${410 + n * 90}" cy="${239 + i * 98}" r="22"/></clipPath>${[0, 1, 2, 3].map((k) => `<clipPath id="pieceClip${i}-${n}-${k}"><circle cx="${444 + n * 90 + (k % 2) * 23}" cy="${224 + i * 98 + Math.floor(k / 2) * 27}" r="11"/></clipPath>`).join('')}`).join('')}`).join('');
