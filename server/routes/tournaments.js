...
export async function renderStandingsImage(t) {
  ...
  const rows = standings.map((s, i) => {
    ...
    const y = 196 + i * 150; const rankColor = i === 0 ? '#ffd452' : i === 1 ? '#c3d2df' : i === 2 ? '#e59765' : '#263349';
    ...
    const avatarArt = avatar ? `<image href="${avatar}" x="92" y="${y + 39}" width="62" height="62" preserveAspectRatio="xMidYMid meet"/>` : `<text x="123" y="${y + 78}" text-anchor="middle" class="initial">${initials}</text>`;
    const deckArt = [0, 1, 2].map((n) => {
      const c = combo(s.player.deck, n); const x = 410 + n * 90; const fallback = xml((c.main?.abbrev || c.main?.name || '?').slice(0, 4).toUpperCase());
      const primary = `<circle cx="${x}" cy="${y + 29}" r="22" class="blade-ring"/>${asset(c.main) ? `<image href="${asset(c.main)}" x="${x - 22}" y="${y + 7}" width="44" height="44" preserveAspectRatio="xMidYMid meet" clip-path="url(#bladeClip${i}-${n})"/>` : `<text x="${x}" y="${y + 33}" text-anchor="middle" class="blade-fallback">${fallback}</text>`}`;
      const extras = c.small.slice(0, 4).map((p, k) => { const sx = x; const sy = y + 80 + k * 18; const label = xml((p.abbrev || p.name || '?').slice(0, 2).toUpperCase()); return `<circle cx="${sx}" cy="${sy}" r="9" class="piece-ring"/>${asset(p) ? `<image href="${asset(p)}" x="${sx - 9}" y="${sy - 9}" width="18" height="18" preserveAspectRatio="xMidYMid meet" clip-path="url(#pieceClip${i}-${n}-${k})"/>` : `<text x="${sx}" y="${sy + 3}" text-anchor="middle" class="piece-fallback">${label}</text>`}`; }).join('');
      const comboLabel = [c.ratchet?.abbrev || c.ratchet?.name, c.bit?.abbrev || c.bit?.name].filter(Boolean).join(' ');
      return primary + (comboLabel ? `<text x="${x}" y="${y + 62}" text-anchor="middle" class="combo-label">${xml(comboLabel)}</text>` : '') + extras;
    }).join('');
    return `<g><rect x="24" y="${y}" width="932" height="138" rx="14" class="row ${i % 2 ? 'row-alt' : ''}"/><circle cx="54" cy="${y + 70}" r="18" fill="${rankColor}"/><text x="54" y="${y + 76}" text-anchor="middle" class="rank">${i + 1}</text><circle cx="123" cy="${y + 70}" r="31" class="avatar-ring"/>${avatarArt}<text x="174" y="${y + 61}" class="name">${displayName}</text><text x="174" y="${y + 84}" class="handle">@${xml(s.player.user.slug)}</text>${deckArt}<text x="687" y="${y + 76}" text-anchor="middle" class="stat-main">${s.points}</text><text x="750" y="${y + 76}" text-anchor="middle" class="stat">${s.wins}</text><text x="804" y="${y + 76}" text-anchor="middle" class="stat">${s.losses}</text><text x="864" y="${y + 76}" text-anchor="middle" class="stat">${Math.round(s.omw * 100)}%</text><text x="928" y="${y + 76}" text-anchor="middle" class="stat">${Math.round(s.oomw * 100)}%</text></g>`;
  }).join('');
  const height = 310 + standings.length * 150;
  const defs = standings.map((_, i) => `<clipPath id="avatarClip${i}"><circle cx="123" cy="${266 + i * 150}" r="31"/></clipPath>${[0, 1, 2].map((n) => `<clipPath id="bladeClip${i}-${n}"><circle cx="${410 + n * 90}" cy="${225 + i * 150}" r="22"/></clipPath>${[0, 1, 2, 3].map((k) => `<clipPath id="pieceClip${i}-${n}-${k}"><circle cx="${410 + n * 90}" cy="${276 + i * 150 + k * 18}" r="9"/></clipPath>`).join('')}`).join('')}`).join('');
