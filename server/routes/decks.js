...
async function renderDeckShareImage(deck) {
  ...
  const comboArt = combos.map((combo, index) => {
    ...
    const byKind = (kind) => combo.find((part) => part.kind === kind);
    const blade = byKind('BLADE');
    const main = blade || byKind('MAIN_BLADE') || combo[0];
    const lock = byKind('LOCK_CHIP');
    const assist = byKind('ASSIST_BLADE');
    const over = byKind('OVER_BLADE');
    const ratchet = byKind('RATCHET');
    const bit = byKind('BIT');
    const isCx = !blade && Boolean(byKind('MAIN_BLADE'));
    const name = (part) => xml(cut(part.displayName || part.name, 17));
    const art = (part, px, py, size, role, mainPiece = false) => {
      if (!part) return '';
      const radius = size / 2;
      return `<circle cx="${px}" cy="${py}" r="${radius}" class="${mainPiece ? 'main-ring' : 'piece-ring'}"/>${assets.get(part.imageUrl) ? `<image href="${assets.get(part.imageUrl)}" x="${px - radius}" y="${py - radius}" width="${size}" height="${size}" preserveAspectRatio="xMidYMid meet"/>` : `<text x="${px}" y="${py + 4}" text-anchor="middle" class="fallback small">${xml(cut(part.abbrev || part.displayName || part.name, 5))}</text>`}<text x="${px}" y="${py + radius + 15}" text-anchor="middle" class="piece-role">${role}</text><text x="${px}" y="${py + radius + 30}" text-anchor="middle" class="part-name small-name">${name(part)}</text>`;
    };
    const top = isCx
      ? `${art(lock, centerX - 76, 292, 58, 'LOCK CHIP')}${art(main, centerX, 292, 92, 'MAIN BLADE', true)}${art(assist, centerX + 76, 292, 58, 'ASSIST BLADE')}`
      : art(main, centerX, 292, 124, 'BLADE', true);
    const lowerY = isCx ? 455 : 468;
    const lower = `${art(over, centerX - 92, lowerY, 54, 'OVER BLADE')}${art(ratchet, centerX - 55, lowerY, 64, 'RATCHET')}${art(bit, centerX + 55, lowerY, 64, 'BIT')}`;
    return `<g><text x="${x + 18}" y="211" class="combo-head">BEY ${index + 1}</text>${top}${lower}</g>`;
  }).join('');
