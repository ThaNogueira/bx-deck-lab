...
fi
{
  ...
  echo "    # Chave RSA + cadeia rooteada em ISRG Root X1: a hierarquia ECDSA nova do Let's Encrypt (Root YE) ainda nao e confiada em todo lugar"
  echo "    tls {"
  echo "        key_type rsa2048"
  echo "        issuer acme {"
  echo "            preferred_chains {"
  echo "                root_common_name \"ISRG Root X1\""
  echo "            }"
  echo "        }"
  echo "    }"
